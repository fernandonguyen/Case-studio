...
(function() {
	...
	p._doCollide = function (collisionType, contact) {
		...
		for(let i = collider1._node.components.length - 1; i >= 0; i--){
			let component = collider1._node.components[i];
			if (component[contactFunc]) {
				component[contactFunc](collider2, collider1);
			}
		}
		for(let i = collider2._node.components.length - 1; i >= 0; i--){
			let component = collider2._node.components[i];
			if (component[contactFunc]) {
				component[contactFunc](collider2, collider1);
			}
		}
